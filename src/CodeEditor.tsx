// ─────────────────────────────────────────────────────────────────────────────
// CodeEditor.tsx — Remotion Typing Engine (Final Regex & Sync Patch)
// ─────────────────────────────────────────────────────────────────────────────
...
function highlight(code: string): string {
  ...
  h = h
    ...
    .replace(/\b([A-Z][a-zA-Z0-9]*)\b/g, '§TYP§$1§END§')
    .replace(/\b([a-zA-Z_][a-zA-Z0-9_]*)(?=\s*:)/g, '§KEY§$1§END§')
    .replace(/\.([a-zA-Z_][a-zA-Z0-9_]*)(?=\(|\[|,|\)|\s|$)/g, '.§MTH§$1§END§');

  // تم تصحيح جميع الـ Regex هنا لضمان عدم تداخل الأكواد
  return h
...
function computeStateAtMs(script: StepType[], initialPreview: PreviewState, elapsedMs: number): EngineState {
  let rawCode   = '';
  let cursorIdx = 0;
  ...
  for (const step of script) {
    ...
    switch (step.a) {
      case 'type': {
        ...
        // تعديل مهم: تحديث الـ Output فوراً بمجرد بدء السطر عشان يكون أسرع
        if (step.out) {
      ...
      case 'delete': {
        ...
        if (step.out) preview = { ...preview, ...step.out };
        rem -= dur;
        break;
      }
      case 'callout': {
...
export const CodeEditor: React.FC<{ lesson: LessonData; startFrame: number }> = ({ lesson, startFrame }) => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  const elapsedMs = Math.max(0, ((frame - startFrame) / fps) * 1000);

  const { rawCode, cursorIdx, preview, callouts } = computeStateAtMs(lesson.script, lesson.initialPreview, elapsedMs);
  ...
  // استخدام رمز مخفي تماماً للمؤشر لمنع تعارضه مع الـ Regex
  const CUR_MARKER = '\uE000';
  const codeWithCursor = rawCode.slice(0, cursorIdx) + CUR_MARKER + rawCode.slice(cursorIdx);
  const codeHtml = highlight(codeWithCursor)
    .replace(CUR_MARKER, cursor)
    .split('\n')
    ...
    <div style={editorWin}>
      ...
      <div style={ewBody}>
        <div style={lineNumbers}>
          {rawCode.split('\n').map((_, i) => <div key={i} style={lineNumStyle}>{i + 1}</div>)}
        </div>
...
const chromBar: React.CSSProperties = {
  background: 'linear-gradient(90deg, #1a1e26, #161b22)', padding: '14px 20px',
  display: 'flex', alignItems: 'center', gap: 10, borderBottom: `1px solid ${T.border}`,
};
const chromTitle: React.CSSProperties = {
  fontFamily: "'JetBrains Mono', monospace", fontSize: 14, color: T.dim, marginLeft: 12, letterSpacing: '.03em',
};
const dot: React.CSSProperties = {
  width: 14, height: 14, borderRadius: '50%', boxShadow: '0 1px 4px rgba(0,0,0,.4)', flexShrink: 0,
};
const ewBody: React.CSSProperties = {
  background: T.surface2, padding: `${EW_BODY_PADDING_TOP}px 24px 20px 0`,
  direction: 'ltr', fontSize: FONT_SIZE_PX, display: 'flex', position: 'relative', minHeight: 80,
};
const lineNumbers: React.CSSProperties = {
  display: 'flex', flexDirection: 'column', alignItems: 'flex-end', padding: '0 16px', minWidth: 52, userSelect: 'none',
};
const lineNumStyle: React.CSSProperties = {
  fontFamily: "'JetBrains Mono', monospace", fontSize: FONT_SIZE_PX - 4, color: T.dim,
  lineHeight: `${LINE_HEIGHT_PX}px`, minHeight: LINE_HEIGHT_PX, opacity: 0.5,
};
const codeLineStyle =
  `font-family:'JetBrains Mono',monospace;font-size:${FONT_SIZE_PX}px;line-height:1.9;letter-spacing:.01em;` +
  `color:${T.text};white-space:pre-wrap;display:block;min-height:${LINE_HEIGHT_PX}px;position:relative;`;
const chip: React.CSSProperties = {
  position: 'absolute', right: 0, background: `linear-gradient(135deg, ${T.warm}, #f87316)`,
  color: '#0a0e14', fontSize: 15, fontWeight: 800, padding: '5px 18px', borderRadius: 32,
  whiteSpace: 'nowrap', boxShadow: '0 4px 16px rgba(245,158,11,.4)', fontFamily: "'Cairo', sans-serif",
  direction: 'rtl', transform: 'translateY(-50%)', zIndex: 10,
};
const previewWrap: React.CSSProperties = {
  border: `1px solid ${T.border}`, borderTop: `1px solid ${T.border}`,
  background: 'linear-gradient(135deg, rgba(124,58,237,.12), rgba(0,212,255,.06))',
  display: 'flex', flexDirection: 'column', gap: 8, padding: '16px 20px', boxShadow: 'inset 0 1px 0 rgba(255,255,255,.04)',
};
const previewLabel: React.CSSProperties = {
  color: T.accent, fontSize: 14, fontFamily: "'JetBrains Mono', monospace", letterSpacing: '.15em', fontWeight: 600,
};
